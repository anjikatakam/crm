...
<body>
    ...
    <div class="contant-body">
        <div class="nav-bar">
			...
			<?php  foreach($countbyopstatus as $opcount) { 
            ...
            <?php  foreach(@$countbyleadstatus  as $lscount) { 
			          if($lscount['lead_status']=="Hot Lead") {
			           @$hotcount=$lscount['count'];
					  } 
					  if($lscount['lead_status']=="Warm Lead") {
			           @$warmcount=$lscount['count'];
					  } 
					  if($lscount['lead_status']=="Cold Lead") {
			          @$coldcount=$lscount['count'];
					  } 
					  if($lscount['lead_status']=="Dead") {
			          @$deadcount=$lscount['count'];
					  }
					  if($lscount['lead_status']=="Won") {
			           @$woncount=$lscount['count'];
					  }
					  if($lscount['lead_status']=="Inactive") {
			           @$inacount=$lscount['count'];
					  }
					  if($lscount['lead_status']=="Negotiating") {
			           @$negocount=$lscount['count'];
					  }
            ...
            <div class="container-fluid">
                ...
                <div class="ticket-item">
					...
					?>
                        ...
                        <div class="t-number">
                            <p class="mb-0"><?php echo $sr."".$ticket['sr_request'];?><br/> <?php echo $ticket['title'];?></p>
...
<!--start show details with comments section start--------------->
    <div class="show-details">
        ...
        <div class="footer">
            <div class="reply">
                <button class="btn-primary" onclick="openForm()" class="replyButton"><i class="fas fa-reply" ></i> Reply </button>
            </div>
        </div>
        <div class="chatBoxDiv" id="myForm" style="display:none;">
		  <form id="" name="" class="" method="post" action="<?php echo base_url();?>Admin/submitAdminListComment"
...
$(".leadStatus").click(function(){
	...
	var lsval=$(this).html();
	 $.ajax({
		    type:"POST",
			url:"<?php echo base_url();?>Admin/getTktsByLeadStatus",
			data: {lsval:lsval},
 ...
 $(".opStatus").click(function(){
	var opStatus=$(this).html();
	 ...
	 $.ajax({
		    type:"POST",
			url:"<?php echo base_url();?>Admin/getTktsByOpStatus",
